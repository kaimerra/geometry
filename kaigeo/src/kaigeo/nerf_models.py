...
# n_harmonic_functions=60, n_hidden_neurons=256):
class NeuralRadianceField(torch.nn.Module):
    ...
    def _get_colors(self, features, rays_directions):
        ...
        if features.shape[:-1] != rays_embedding.shape[:-1]:
            rays_embedding_expand = rays_embedding[..., None, :].expand(
                *spatial_size, rays_embedding.shape[-1]
            )
        else:
    ...
    def sample_densities(self, pts):
        embeds = self.harmonic_embedding(pts)
        # embeds.shape = [minibatch x ... x self.n_harmonic_functions*6]
        ...
        # pick direction
        direction = torch.tensor([[1.0, 0.0, 0.0]], device=features.device)
        directions = torch.tile(direction, (len(pts), 1))
